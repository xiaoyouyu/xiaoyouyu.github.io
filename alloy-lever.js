...
;(function (root, factory) {
    ...
    var AlloyLever = {}
    AlloyLever.settings = {
        // cdn:App.cdnUrl+'libs/log/vconsole.min.js',
        reportUrl: null,
        reportPrefix: '',
        reportKey: 'msg',
        otherReport: null,
    ...
    var parameter = getParameter('isVLog')
    ...
    function filter(log){
        if(log.indexOf('日志')>-1){
           log = '<span style=" font-weight:bold;color:green;"><--------- 日志 ---------></span>'; 
        }else if(log.indexOf('font-weight:bold;color:green;')>-1){
            log = '';
        }else if(log.indexOf('提交的数据')>-1||log.indexOf('返回的数据如下')>-1){
            log = log.replace('%c','');
            log = '<span style=" font-weight:bold;color:green;">'+log+'<span>';
        }else if(log.indexOf('The request data is')>-1){
            log = log.replace('The request data is','<span style=" font-weight:bold;color:green;">请求数据如下:</span><br/>');
        }
        return log;
    }
